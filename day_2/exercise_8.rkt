...
#|
Exercise 8. Modify the definition of split-ct from lecture so that it can also deal with specifications that do not provide literal constants for start and end. When a non-literal start or end is provided, the range check should happen at run time.
...
(define-syntax (split-ct stx)
  (syntax-parse stx
    [(_ tags start:expr end:expr [name step:byte (~optional convert)] ...)
     ; ———————————
     ; the static error checking 
     #:do [(define end-int  (syntax-e #'(values end)))
           (define step-int (+ start (sum #'(step ...))))]
     #:fail-unless (< step-int end-int) "index out of range"
     ; ———————————
     #`(let ([i start])
         (let*-values ([(i name) (values (+ i step) (extract tags i (+ i step -1)))]
